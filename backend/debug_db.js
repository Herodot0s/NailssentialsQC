async function main() {
  ...
  apts.forEach((a) => {
    console.log(
      `ID=${a.id} date=${a.appointment_date} status=${a.status} customer=${a.customer?.full_name || 'N/A'}`,
    );
    a.items.forEach((i) =>
      console.log(
        `  item: staff_id=${i.staff_id} staff=${i.staff?.full_name} service=${i.service?.name || 'N/A'}`,
      ),
    );
  });
}
