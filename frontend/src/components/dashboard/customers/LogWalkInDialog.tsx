  ...
  useEffect(() => {
    if (open) {
      const fetchData = async () => {
        setIsLoading(true);
        try {
          const [serviceRes, staffRes] = await Promise.all([getServices(), getAllStaff()]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl border border-hairline shadow-2xl rounded-md p-0 overflow-hidden bg-surface-card">
        <div className="bg-primary p-10 text-white">
          <DialogHeader>
            <DialogTitle className="display-lg text-white">
              Walk-in <span className="italic opacity-80">Registration</span>
            </DialogTitle>
            <DialogDescription className="text-white/70 body-md mt-2">
              Instant service logging for on-site clients.
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        {isLoading ? (
          ...
          <form
            ...
            className="p-10 space-y-8 max-h-[60vh] overflow-y-auto no-scrollbar"
          >
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {selectedItems.map((item, index) => (
                  <div
                    key={index}
                    className="p-6 bg-surface-soft/50 border border-hairline-soft rounded-md space-y-4 relative group"
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(val) => updateItem(index, 'serviceId', val || '')}
                        >
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          value={item.staffId}
                          onValueChange={(val) => updateItem(index, 'staffId', val || '')}
                        >
                          <SelectTrigger className="rounded-md border-hairline h-12 bg-white">
                            <SelectValue placeholder="Select Technician">
                              {
                                staffList.find((s) => s.staffProfileId.toString() === item.staffId)
                                  ?.fullName
                              }
                            </SelectValue>
                          </SelectTrigger>
                          <SelectContent className="rounded-md border-hairline shadow-xl">
                            {staffList
                              ...
                              .map((s) => (
                                <SelectItem key={s.id} value={s.staffProfileId.toString()}>
                                  {s.fullName}
                                </SelectItem>
                              ))}
                          </SelectContent>
