                ...
                <TableCell>
                  <div className="flex gap-0.5">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`h-3 w-3 ${i < review.rating ? 'text-primary fill-primary' : 'text-stone stroke-[1]'}`}
                      />
                    ))}
                  </div>
                </TableCell>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {(review.tags as string[]).map((tag) => (
                      <Badge
                        key={tag}
                        variant="outline"
                        className="rounded-full caption-xs border-hairline bg-surface-soft/50 px-2 py-0 text-mute"
                      >
                        {tag}
                      </Badge>
                    ))}
