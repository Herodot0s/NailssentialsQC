...
const StaffTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="bg-white shadow-sm border border-gray-100 p-4" style={{ borderRadius: 0 }}>
      ...
      <p className="text-sm text-muted-foreground">{formatCurrency(d?.revenue)} revenue, {d?.serviceCount} services</p>
    </div>
  );
};
    ...
    <div className="space-y-8">
      ...
      <Card className="rounded-none border-none shadow-sm bg-white">
        <CardHeader className="border-b border-gray-50">
          <CardTitle className="font-serif text-2xl font-light !uppercase-none !tracking-normal !text-2xl">Staff Revenue Ranking</CardTitle>
        </CardHeader>
        <CardContent className="pt-6">
          <ResponsiveContainer width="100%" height={Math.max(300, safeData.length * 48)}>
            <BarChart layout="vertical" data={safeData}>
              <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#f0f0f0" />
              <XAxis type="number" axisLine={false} tickLine={false} tick={{ fontSize: 12 }} tickFormatter={formatCompact} />
      ...
      <Card className="rounded-none border-none shadow-sm bg-white">
        <CardHeader className="border-b border-gray-50">
          <CardTitle className="font-serif text-2xl font-light !uppercase-none !tracking-normal !text-2xl">Performance Leaderboard</CardTitle>
        </CardHeader>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground text-left px-6 py-4">Staff Name</th>
                <th className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground text-left px-6 py-4">Revenue Generated</th>
                <th className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground text-left px-6 py-4">Commission Earned</th>
                <th className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground text-left px-6 py-4">Services Completed</th>
              </tr>
            </thead>
            <tbody>
                        ...
                        <td colSpan={5}>
                          ...
                          >
                            <div className="bg-gray-50/30 px-6 py-4">
                              <p className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground mb-3">Category Breakdown</p>
                              <div className="space-y-2">
                                {Object.entries(staff.categoryBreakdown).map(([cat, amount]) => {
                                  const maxAmount = Math.max(...Object.values(staff.categoryBreakdown));
                                  const pct = maxAmount > 0 ? (amount / maxAmount) * 100 : 0;
                                  const colorIdx = Object.keys(CHART_COLORS).indexOf(cat);
                                  const color = CHART_COLORS[cat] || FALLBACK_COLORS[colorIdx >= 0 ? colorIdx : 0];
                                  return (
                                    <div key={cat} className="flex items-center gap-3">
                                      ...
                                      <span className="text-xs font-semibold w-20 text-right">{formatCurrency(amount)}</span>
                                    </div>
                                  );
                                })}
                              </div>
