...
export const MobileCheckIn: React.FC<MobileCheckInProps> = (props) => {
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 z-50 bg-[#eeefe9] flex flex-col items-center justify-center p-6 md:hidden">
          <div className="space-y-[40px] text-center w-full max-w-sm relative">
            ...
            <div className="bg-white rounded-md p-12 flex flex-col items-center justify-center border border-[#bfc1b7] relative overflow-hidden group">
              ...
              <span suppressHydrationWarning className="text-[12px] text-[#4d4f46] font-bold uppercase tracking-[0.3em] mb-4 relative z-10">
                {weekday}
              </span>
              <span suppressHydrationWarning className="text-7xl font-black text-[#23251d] mb-4 relative z-10 tabular-nums tracking-tighter filter drop-shadow-[0_2px_2px_rgba(0,0,0,0.1)]">
                {time}
              </span>

              {scheduledShiftStr && (
                <div className="flex flex-col items-center mb-6 relative z-10">
                  <p className="text-[9px] text-[#6c6e63] font-black uppercase tracking-[0.2em] mb-1 opacity-60">Scheduled Shift</p>
                  <p className="text-[11px] font-bold text-[#23251d] tracking-wide">{scheduledShiftStr}</p>
                </div>
              )}

              <Badge
                className={`rounded-none border text-[10px] uppercase tracking-widest font-black px-6 py-2 shadow-none relative z-10 transition-colors duration-500 ${isCheckedIn
      ...
      {/* Compact card mode */}
      <Card
        className={`lg:col-span-4 rounded-md border border-[#bfc1b7] shadow-none bg-white overflow-hidden transition-all duration-500 ${showMobileOverlay ? 'hidden md:block' : 'block opacity-100 translate-y-0'
          }`}
      >
        ...
        <CardContent className="p-8 flex flex-col items-center gap-10 relative">
          ...
          <div className="absolute inset-0 pointer-events-none opacity-[0.02]" style={{ filter: 'url(#analog-grain)' }} />
          ...
          <div className="w-full space-y-8 relative z-10">
            <div className="flex flex-col items-center justify-center p-10 bg-[#eeefe9]/30 rounded-md border border-[#bfc1b7] relative overflow-hidden">
              ...
              <span suppressHydrationWarning className="text-[11px] text-[#6c6e63] font-bold uppercase tracking-[0.2em] mb-2">
                {weekday}
              </span>
              <span suppressHydrationWarning className="text-6xl font-black text-[#23251d] mb-6 tabular-nums tracking-tighter">
                {time}
              </span>

              <div className="flex flex-col items-center gap-4 w-full">
                <Badge
                  className={`rounded-none border text-[10px] uppercase tracking-[0.2em] font-black px-5 py-2 flex items-center gap-2 shadow-none transition-colors duration-500 ${isCheckedIn
                    ? 'bg-[#d9eddf] text-[#2c8c66] border-[#2c8c66]/20'
                ...
                {isCheckedIn && (
                  <div className={`mt-2 w-full flex flex-col items-center ${!scheduledShiftStr ? 'pt-4 border-t border-[#bfc1b7]/30' : ''}`}>
            ...
            <div className="space-y-6">
              ...
              {isCheckedIn && props.checkInTime && (
                <div className="flex flex-col items-center gap-1">
                  <p suppressHydrationWarning className="text-[10px] text-[#6c6e63] font-bold uppercase tracking-[0.2em] opacity-80">
