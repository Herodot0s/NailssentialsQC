...
const DEFAULTS: SiteSettingsData = {
  hero: {
    ...
    subheadline:
      'Discover a haven of serenity where expert craftsmanship meets premium self-care. Welcome to the NailssentialsQC sanctuary.',
    bg_image_url:
      'https://images.unsplash.com/photo-1600334129128-685c4582f98c?auto=format&fit=crop&q=80&w=2070',
    button_label: 'Book Your Sanctuary',
  },
  signature: {
    label: 'Signature Experience',
    headline: 'The Nailssentials Ritual',
    body: 'Step into a world where time slows down. Our signature ritual combines aromatherapy, precision technique, and an atmosphere of absolute luxury to revitalize your spirit.',
    link_label: 'Discover the Menu',
    bg_image_url:
      'https://images.unsplash.com/photo-1519014816548-bf5fe059798b?auto=format&fit=crop&q=80&w=2070',
  },
...
const SECTIONS: SectionConfig[] = [
  {
    key: 'hero',
    title: 'Hero Section',
    description: 'The first thing visitors see — tagline, headline, and call-to-action.',
    fields: [
  ...
  },
  {
    ...
    description: "Highlight your salon's unique offering below the hero.",
    fields: [
      { key: 'label', label: 'Section Label' },
      { key: 'headline', label: 'Headline' },
      { key: 'body', label: 'Body Paragraph', type: 'textarea' },
      { key: 'link_label', label: 'Link Label' },
      { key: 'bg_image_url', label: 'Image URL' },
    ],
  },
  {
    key: 'footer',
    title: 'Footer Call-to-Action',
    description: 'The closing section that drives registration.',
    fields: [
      { key: 'headline', label: 'Headline' },
  ...
  },
  {
    ...
    description: 'Phone, address, and hours — hidden on landing page if all fields are empty.',
    fields: [
...
export const LandingPageEditor: React.FC = () => {
  ...
  useEffect(() => {
    getCmsSettings()
      .then((r) => {
        if (r.data.success && r.data.data) setSettings((s) => ({ ...s, ...r.data.data }));
      })
  ...
  const handleSave = async (section: SectionConfig) => {
    ...
    const settingsArr = section.fields.map((f) => ({
      section: section.key,
      key: f.key,
    ...
    <div className="space-y-6">
      {loadError && (
        <p className="text-sm text-destructive">
          Failed to load current content. Showing defaults.
        </p>
      )}
          ...
          <Card key={section.key} className="rounded-none border-gray-100">
            ...
            <CardFooter className="justify-end border-t border-gray-50 pt-4">
              <Button
                ...
                className="rounded-none px-6 text-[10px] uppercase tracking-widest font-bold h-10"
              >
                {saving === section.key ? (
                  <>
