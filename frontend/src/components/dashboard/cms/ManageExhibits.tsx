...
const ManageExhibits: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm('Are you sure you want to remove this exhibit from the public gallery?')) return;
  ...
  if (isLoading && exhibits.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-64px)] gap-4 bg-gray-50/30">
        ...
        <p className="text-[10px] tracking-[0.3em] uppercase font-bold text-muted-foreground">Loading Gallery Assets...</p>
      </div>
    );
  }
  ...
  return (
    <div className="container max-w-7xl mx-auto py-12 px-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <div>
          <p className="text-[10px] tracking-[0.4em] uppercase font-bold text-primary mb-2">Content Management</p>
          ...
          <p className="text-muted-foreground mt-2 max-w-md text-sm font-light">
            Curate the public gallery by showcasing the salon's best work. Upload high-res images and assign artist credits.
          </p>
        </div>
        <Button onClick={() => setIsFormOpen(true)} className="rounded-none gap-2 px-8 h-14 text-[11px] uppercase font-bold tracking-[0.2em] shadow-xl shadow-primary/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {exhibits.map((ex) => (
          <Card key={ex.id} className="rounded-none border-none shadow-sm group overflow-hidden bg-white">
            ...
            <CardHeader className="p-5 space-y-3">
               <div className="flex justify-between items-start">
                  <div className="space-y-1">
                     ...
                     <CardTitle className="font-serif text-lg font-bold leading-tight group-hover:text-primary transition-colors">{ex.title}</CardTitle>
                  </div>
                  {ex.service && (
                    <Badge variant="outline" className="rounded-none border-gray-100 text-[8px] uppercase font-bold tracking-tighter bg-gray-50/50">
                      {ex.service.name}
                    </Badge>
                  )}
               </div>
        ...
        {exhibits.length === 0 && !isLoading && (
          <div className="col-span-full py-32 text-center bg-gray-50/50 border border-dashed border-gray-200">
             ...
             <p className="text-muted-foreground font-serif italic text-lg">No exhibits published yet.</p>
             <p className="text-[10px] uppercase font-bold tracking-[0.2em] text-muted-foreground/40 mt-2">Start your gallery by clicking "Publish New Work"</p>
          </div>
        )}
      </div>
        ...
        <DialogContent className="sm:max-w-2xl rounded-none border-none shadow-2xl p-0 overflow-hidden">
          <div className="bg-primary p-10 text-white">
             <DialogHeader className="space-y-4">
                <p className="text-[10px] uppercase font-bold tracking-[0.4em] text-white/60">New Submission</p>
                <DialogTitle className="font-serif text-5xl font-light tracking-tight">Create <span className="italic">Exhibit</span></DialogTitle>
