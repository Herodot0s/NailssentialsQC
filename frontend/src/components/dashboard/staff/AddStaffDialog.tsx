...
export const AddStaffDialog: React.FC<AddStaffDialogProps> = ({
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.data.success) {
        onFormChange({ ...form, profilePictureUrl: res.data.data.url });
      }
    } catch (err) {
  ...
  const isSectionComplete = (type: 'identity' | 'credentials' | 'compliance' | 'compensation') => {
    switch (type) {
      ...
      case 'credentials':
        return !!(form.email && form.phone && form.role);
      case 'compliance':
            ...
            <div className="space-y-10">
              ...
              >
                ...
                <div className="space-y-5">
                  ...
                  <div className="relative group">
                    <div className="flex items-center gap-4 p-4 bg-[#eeefe9]/50 border border-[#bfc1b7] rounded-[6px] transition-all hover:border-[#6c6e63]">
                      <Avatar className="h-16 w-16 rounded-[4px] border border-[#bfc1b7] bg-white">
                        <AvatarImage src={form.profilePictureUrl} className="object-cover" />
                        <AvatarFallback className="text-[#23251d] font-bold text-xl">
                          {form.fullName?.charAt(0) || '?'}
                        </AvatarFallback>
                      </Avatar>
                      <div className="flex-1 space-y-2">
                        <Label className="text-[10px] uppercase font-[800] tracking-[0.1em] text-[#6c6e63]">
                        ...
                        <div className="flex gap-2">
                          ...
                          {form.profilePictureUrl && (
                            <Button
                              type="button"
                              variant="ghost"
                              onClick={() => onFormChange({ ...form, profilePictureUrl: '' })}
                              className="h-9 px-3 text-[#cd4239] hover:bg-[#cd4239]/10 hover:text-[#cd4239]"
                            >
                              <X className="h-4 w-4" />
                            </Button>
                          )}
