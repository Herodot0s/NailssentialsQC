  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-12 mb-12">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#6c6e63] mb-1">
            Employee
          </p>
      ...
      <div className="grid grid-cols-4 gap-4 mb-12">
        <div className="bg-[#fcfcfa] border border-[#bfc1b7] p-4 rounded-md">
          <p className="text-[9px] font-black uppercase tracking-widest text-[#6c6e63] mb-1">
            Gross Pay
          </p>
          <p className="text-lg font-bold">
            ₱{(Number(payroll.base_pay) + Number(payroll.commissions)).toLocaleString()}
          </p>
        </div>
        <div className="bg-[#fcfcfa] border border-[#bfc1b7] p-4 rounded-md">
          <p className="text-[9px] font-black uppercase tracking-widest text-[#6c6e63] mb-1">
          ...
          <p className="text-lg font-bold text-[#B8794E]">
            ₱{Number(payroll.commissions).toLocaleString()}
          </p>
        </div>
        <div className="bg-[#fcfcfa] border border-[#bfc1b7] p-4 rounded-md">
          <p className="text-[9px] font-black uppercase tracking-widest text-[#6c6e63] mb-1">
          ...
          <p className="text-lg font-bold text-[#cd4239]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        {/* Earnings Table */}
        <div>
          <h3 className="text-[12px] font-black uppercase tracking-widest mb-4 pb-2 border-b border-[#bfc1b7]">
            Earnings
          </h3>
            ...
            <table className="w-full text-sm">
              <tbody>
                {deductions.map((item, idx) => (
                  <tr key={idx} className="border-b border-[#bfc1b7]/30">
                    <td className="py-3 font-medium text-[#4d4f46]">{item.component_name}</td>
                    <td className="py-3 text-right font-bold text-[#cd4239] tabular-nums">
                      -₱{item.amount.toLocaleString()}
                    </td>
                  </tr>
                ))}
          ...
          <div className="grid grid-cols-7 gap-2">
            {Object.entries(payroll.daily_breakdown).map(([date, sales]) => (
              <div key={date} className="text-center p-2 border border-[#bfc1b7]/50 rounded-md">
                <p className="text-[8px] font-black uppercase text-[#6c6e63]">
                  {format(new Date(date), 'EEE')}
                </p>
                <p className="text-[10px] font-bold text-[#4d4f46] my-1">
