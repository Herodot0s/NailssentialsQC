...
const CustomerAppointments: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    ...
    switch (s) {
      case 'completed':
        return (
          <Badge className="bg-success-color/10 text-success-color border-none rounded-none text-[8px] uppercase tracking-widest font-bold px-3">
            Completed
          </Badge>
        );
      case 'cancelled':
        return (
          <Badge
            variant="outline"
            className="text-muted-foreground border-muted-foreground rounded-none text-[8px] uppercase tracking-widest px-3"
          >
            Cancelled
          </Badge>
        );
      case 'confirmed':
        return (
          <Badge className="bg-info-color/10 text-info-color border-none rounded-none text-[8px] uppercase tracking-widest font-bold px-3">
            Confirmed
          </Badge>
        );
      case 'pending':
        return (
          <Badge className="bg-primary/5 text-primary border-none rounded-none text-[8px] uppercase tracking-widest font-bold px-3">
            Incoming
          </Badge>
        );
      default:
        return (
          <Badge
            variant="outline"
            className="rounded-none text-[8px] uppercase tracking-widest px-3"
          >
            {status}
          </Badge>
        );
        ...
        <div className="text-center py-32 bg-primary-ultra/10 border border-dashed border-primary/10 animate-in fade-in duration-1000">
          <div className="space-y-6 max-w-sm mx-auto">
            ...
            <p className="text-muted-foreground text-sm font-light">
              Begin your self-care journey with us by scheduling your first ritual.
            </p>
            <Link to="/booking">
          ...
          <div className="bg-primary p-10 text-white">
            <DialogHeader>
              <DialogTitle className="font-serif text-4xl font-light italic">
                Ritual <span className="not-italic">Feedback</span>
              </DialogTitle>
          ...
          <div className="p-10 space-y-8 bg-white">
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                {[1, 2, 3, 4, 5].map((star) => (
                  ...
                  >
                    <Star
                      className={`h-8 w-8 ${star <= reviewForm.rating ? 'text-primary fill-primary' : 'text-muted stroke-[1]'}`}
                    />
                  </button>
                ))}
              </div>
            ...
            <DialogFooter className="pt-6">
              ...
              <Button
                ...
                className="rounded-none px-12 h-14 font-bold uppercase tracking-widest text-[10px]"
              >
                Publish Review
              </Button>
            </DialogFooter>
      ...
      <Dialog open={showCancelModal} onOpenChange={setShowCancelModal}>
        <DialogContent className="max-w-md border-none shadow-2xl rounded-none p-0 overflow-hidden">
          <div className="bg-destructive p-10 text-white">
            <DialogHeader>
              <DialogTitle className="font-serif text-4xl font-light italic">
                Cancel <span className="not-italic">Ritual</span>
              </DialogTitle>
              <DialogDescription className="text-white/70 font-light mt-2">
                We understand plans change. Please let us know why you're cancelling so we can
                improve our service.
              </DialogDescription>
            </DialogHeader>
          </div>
          <div className="p-10 space-y-8 bg-white">
            <div className="space-y-4">
              ...
              <p className="text-[9px] text-muted-foreground uppercase tracking-widest leading-relaxed">
                Note: Cancellations within 24 hours of the ritual may be subject to our standard
                policy.
              </p>
            </div>
