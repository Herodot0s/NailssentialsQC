...
const Gallery: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const cats = [
          'All',
          ...new Set(data.map((ex: Exhibit) => ex.service?.name).filter(Boolean) as string[]),
        ];
        setCategories(cats);
      } catch (err) {
  ...
  const filteredExhibits =
    activeFilter === 'All' ? exhibits : exhibits.filter((ex) => ex.service?.name === activeFilter);

  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh] gap-6">
        <Loader2 className="h-12 w-12 animate-spin text-primary stroke-[1]" />
        <p className="text-[10px] tracking-[0.4em] uppercase font-bold text-muted-foreground animate-pulse">
          ...
          <div className="max-w-3xl space-y-6">
            ...
            >
              ...
              <h1 className="font-serif text-6xl md:text-8xl font-light text-foreground leading-[1.1] tracking-tight">
                Nail Art <br />
                <span className="italic text-primary/40">Exhibit</span>
              </h1>
            </motion.div>
            ...
            >
              A curated collection of our finest work, showcasing the intersection of technical
        ...
        <div className="container max-w-7xl mx-auto">
          ...
          {filteredExhibits.length === 0 && (
            <div className="py-40 text-center space-y-6">
              ...
              <h3 className="font-serif text-2xl font-light italic text-muted-foreground">
